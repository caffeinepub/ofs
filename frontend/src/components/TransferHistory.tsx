...
export default function TransferHistory({ initialSubTab }: TransferHistoryProps) {
  ...
  const [activeTab, setActiveTab] = useState<'received' | 'sent'>(initialSubTab || 'received');

  // Update active tab when initialSubTab prop changes
  useEffect(() => {
    if (initialSubTab && (initialSubTab === 'received' || initialSubTab === 'sent')) {
  ...
  return (
    <div className="space-y-4">
      ...
      <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as 'received' | 'sent')} className="space-y-4">
