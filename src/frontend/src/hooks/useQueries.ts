      ...
      const profiles = await Promise.all(
        users.map(async (user) => {
          try {
            ...
            console.error(
              `Failed to fetch profile for ${user.toString()}:`,
              error,
            );
  ...
  return useMutation({
    ...
    onMutate: async (newTransfer) => {
      ...
      const previousTransfers = queryClient.getQueryData([
        "transferHistory",
        newTransfer.sender.toString(),
      ]);

      // Optimistically update to the new value
      queryClient.setQueryData(
        ["transferHistory", newTransfer.sender.toString()],
        (old: TransferRecordData[] | undefined) => {
    ...
    onError: (_err, newTransfer, context) => {
      // Rollback on error
    ...
    onSettled: (_data, _error, variables) => {
      ...
      queryClient.invalidateQueries({
        queryKey: ["transferHistory", variables.sender.toString()],
      });
      queryClient.invalidateQueries({
        queryKey: ["transferHistory", variables.receiver.toString()],
      });
    },
  });
}
